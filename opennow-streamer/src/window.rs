...
pub fn run(session: SharedSession, media_rx: Receiver<MediaEvent>, width: u32, height: u32) -> anyhow::Result<()> {
    ...
    while running {
        ...
        canvas.copy(&texture, None, Some(Rect::new(0, 0, out_w, out_h))).ok();
        canvas.present();
...
fn map_modifiers(mods: Mod) -> u16 {
    let mut flags = 0_u16;
    if mods.intersects(Mod::LSHIFTMOD | Mod::RSHIFTMOD) { flags |= 0x01; }
    if mods.intersects(Mod::LCTRLMOD | Mod::RCTRLMOD) { flags |= 0x02; }
    if mods.intersects(Mod::LALTMOD | Mod::RALTMOD) { flags |= 0x04; }
    if mods.intersects(Mod::LGUIMOD | Mod::RGUIMOD) { flags |= 0x08; }
    if mods.intersects(Mod::CAPSMOD) { flags |= 0x10; }
    if mods.intersects(Mod::NUMMOD) { flags |= 0x20; }
    flags
}
...
fn map_controller_buttons(controller: &sdl2::controller::GameController) -> u16 {
    ...
    if controller.button(ControllerButton::DPadUp) { buttons |= 0x0001; }
    if controller.button(ControllerButton::DPadDown) { buttons |= 0x0002; }
    if controller.button(ControllerButton::DPadLeft) { buttons |= 0x0004; }
    if controller.button(ControllerButton::DPadRight) { buttons |= 0x0008; }
    if controller.button(ControllerButton::Start) { buttons |= 0x0010; }
    if controller.button(ControllerButton::Back) { buttons |= 0x0020; }
    if controller.button(ControllerButton::LeftStick) { buttons |= 0x0040; }
    if controller.button(ControllerButton::RightStick) { buttons |= 0x0080; }
    if controller.button(ControllerButton::LeftShoulder) { buttons |= 0x0100; }
    if controller.button(ControllerButton::RightShoulder) { buttons |= 0x0200; }
    if controller.button(ControllerButton::Guide) { buttons |= 0x0400; }
    if controller.button(ControllerButton::A) { buttons |= 0x1000; }
    if controller.button(ControllerButton::B) { buttons |= 0x2000; }
    if controller.button(ControllerButton::X) { buttons |= 0x4000; }
    if controller.button(ControllerButton::Y) { buttons |= 0x8000; }
    buttons
}

fn axis_to_u8(value: i16) -> u8 {
    (((value.max(0) as f32) / 32767.0) * 255.0).round().clamp(0.0, 255.0) as u8
}

fn map_scancode(code: Scancode) -> Option<(u16, u16)> {
    Some(match code {
        Scancode::A => (0x41, 0x04), Scancode::B => (0x42, 0x05), Scancode::C => (0x43, 0x06),
        Scancode::D => (0x44, 0x07), Scancode::E => (0x45, 0x08), Scancode::F => (0x46, 0x09),
        Scancode::G => (0x47, 0x0A), Scancode::H => (0x48, 0x0B), Scancode::I => (0x49, 0x0C),
        Scancode::J => (0x4A, 0x0D), Scancode::K => (0x4B, 0x0E), Scancode::L => (0x4C, 0x0F),
        Scancode::M => (0x4D, 0x10), Scancode::N => (0x4E, 0x11), Scancode::O => (0x4F, 0x12),
        Scancode::P => (0x50, 0x13), Scancode::Q => (0x51, 0x14), Scancode::R => (0x52, 0x15),
        Scancode::S => (0x53, 0x16), Scancode::T => (0x54, 0x17), Scancode::U => (0x55, 0x18),
        Scancode::V => (0x56, 0x19), Scancode::W => (0x57, 0x1A), Scancode::X => (0x58, 0x1B),
        Scancode::Y => (0x59, 0x1C), Scancode::Z => (0x5A, 0x1D),
        Scancode::Num1 => (0x31, 0x1E), Scancode::Num2 => (0x32, 0x1F), Scancode::Num3 => (0x33, 0x20),
        Scancode::Num4 => (0x34, 0x21), Scancode::Num5 => (0x35, 0x22), Scancode::Num6 => (0x36, 0x23),
        Scancode::Num7 => (0x37, 0x24), Scancode::Num8 => (0x38, 0x25), Scancode::Num9 => (0x39, 0x26),
        Scancode::Num0 => (0x30, 0x27), Scancode::Return => (0x0D, 0x28), Scancode::Escape => (0x1B, 0x29),
        Scancode::Backspace => (0x08, 0x2A), Scancode::Tab => (0x09, 0x2B), Scancode::Space => (0x20, 0x2C),
        Scancode::Left => (0x25, 0x50), Scancode::Right => (0x27, 0x4F), Scancode::Up => (0x26, 0x52), Scancode::Down => (0x28, 0x51),
        Scancode::LShift => (0xA0, 0xE1), Scancode::RShift => (0xA1, 0xE5), Scancode::LCtrl => (0xA2, 0xE0), Scancode::RCtrl => (0xA3, 0xE4),
        Scancode::LAlt => (0xA4, 0xE2), Scancode::RAlt => (0xA5, 0xE6), Scancode::LGui => (0x5B, 0xE3), Scancode::RGui => (0x5C, 0xE7),
        Scancode::F1 => (0x70, 0x3A), Scancode::F2 => (0x71, 0x3B), Scancode::F3 => (0x72, 0x3C), Scancode::F4 => (0x73, 0x3D),
        Scancode::F5 => (0x74, 0x3E), Scancode::F6 => (0x75, 0x3F), Scancode::F7 => (0x76, 0x40), Scancode::F8 => (0x77, 0x41),
        Scancode::F9 => (0x78, 0x42), Scancode::F10 => (0x79, 0x43), Scancode::F11 => (0x7A, 0x44), Scancode::F12 => (0x7B, 0x45),
        _ => return None,
    })
}
