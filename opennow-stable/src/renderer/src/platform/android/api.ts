...
async function cleanupRecordingDraft(state: RecordingDraft): Promise<void> { await state.pendingWrite.catch(() => undefined); await Filesystem.deleteFile({ path: state.filePath, directory: Directory.Data }).catch(() => undefined); }
